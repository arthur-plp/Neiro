...
> **Résultat de la vérification (22/09/2026) — risque levé, aucun repli nécessaire.** `@serwist/turbopack@9.5.12` + `serwist@9.5.12` + `esbuild@0.28.2` sur Next 16.3.5 : le build passe et bundle le service worker (11 entrées de précache, 586 KiB), `/serwist/sw.js` est servi en `application/javascript` avec l'en-tête `Service-Worker-Allowed: /`, et dans le navigateur `navigator.serviceWorker.getRegistrations()` retourne un enregistrement actif (`state: "activated"`) de scope `http://localhost:3000/` — la racine, donc tout le site — avec le cache `serwist-precache-v2` créé. Deux points à retenir pour l'étape 5 : le service worker est exposé par un route handler (`app/serwist/[path]/route.ts`) et non par un fichier dans `public/`, et le `SerwistProvider` s'insère dans `app/layout.tsx`. Le spike a été retiré intégralement.
